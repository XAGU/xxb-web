 ...
 */
@Validated
@RequestMapping("system/allLog/")
@RestController
public class SysAllLogController extends BaseController {
    /**
     * Describe: 基础路径
     */
    private static String MODULE_PATH = "system/allLog/";
     ...
     */
    @GetMapping("main")
    @PreAuthorize("hasPermission('/system/allLog/select','sys:allLog:select')")
    ...
    public ResuTable data(PageDomain pageDomain, SysLog sysLog, @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") Date minOperatime, @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") Date maxOperatime) {
        PageInfo<SysLog> pageInfo = sysLogService.pageSelect(sysLog, minOperatime, maxOperatime, pageDomain);
